...
atterr0=[10;0;0]*arcdeg;%注意这里旋转顺序zxy
...
%% 相对导航
atterr0_1=[0;0;0]*arcdeg;%注意这里旋转顺序zxy 不准确的初始角
...
qnb_vs=m2qua(Cmserr_1');%初始姿态 不准确的初始角
vn_vs=vn0;
posSINS_vs=pos0;
...
%     [rz,rx,ry]=dcm2angle(q2mat(qnb_vm),'zxy');%转换到传统的坐标
%     [rz1,rx1,ry1]=dcm2angle(q2mat(qnb_vs),'zxy');%转换到传统的坐标
%     SINS_Ret_VLF(kk,:)=...
...
% 对比滤波估计姿态误差
msplot(411, SINS_Ret_VLF(:,22), SINS_Ret_VLF(:,1:3)/arcdeg,'Att / ( ° )'); legend('\it\theta', '\it\gamma', '\it\psi')%无误差纯惯主惯导结果
msplot(412, SINS_Ret_VLF(:,22), SINS_Ret_VLF(:,10:12)/arcdeg,'Att / (° )'); legend('\it\theta', '\it\gamma', '\it\psi')%无误差纯惯主惯导结果
