...
close all
flag.EnReli=true;
flag.Ceil=1;flag.flagSINS_M=1;flag.flagSINS_N=2;flag.figureFlag=1;myfigure;
% 绘制相对导航子惯1姿态结果
...
Rerr=my_getSErr(SINSFR{1,1}.Rall(1:end,:)'-SINS{1,1}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,1}.attall(:,1:end)-SINS{1,1}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSFR{1,1}.Rall(1:end-1,:)'-SINS{1,1}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,1}.attall(:,1:end-1)-SINS{1,1}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1/(Smove{1,1}.f*nts);
Rerr=my_getSErr(SINSR{1,1}.Rall(1:end,:)'-SINS{1,1}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,1}.attall(:,1:end)-SINS{1,1}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSR{1,1}.Rall(1:end-1,:)'-SINS{1,1}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,1}.attall(:,1:end-1)-SINS{1,1}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
%%
disp "SINS2"
errlen=1/(Smove{1,2}.f*nts);
Rerr=my_getSErr(SINSFR{1,2}.Rall(1:end,:)'-SINS{1,2}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,2}.attall(:,1:end)-SINS{1,2}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSFR{1,2}.Rall(1:end-1,:)'-SINS{1,2}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,2}.attall(:,1:end-1)-SINS{1,2}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1/(Smove{1,2}.f*nts);
Rerr=my_getSErr(SINSR{1,2}.Rall(1:end,:)'-SINS{1,2}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,2}.attall(:,1:end)-SINS{1,2}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSR{1,2}.Rall(1:end-1,:)'-SINS{1,2}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,2}.attall(:,1:end-1)-SINS{1,2}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
%%
disp "SINS3"
errlen=1/(Smove{1,3}.f*nts);
Rerr=my_getSErr(SINSFR{1,3}.Rall(1:end,:)'-SINS{1,3}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,3}.attall(:,1:end)-SINS{1,3}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSFR{1,3}.Rall(1:end-1,:)'-SINS{1,3}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSFR{1,3}.attall(:,1:end-1)-SINS{1,3}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('滤波终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1/(Smove{1,3}.f*nts);
Rerr=my_getSErr(SINSR{1,3}.Rall(1:end,:)'-SINS{1,3}.R(:,1:2:end),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,3}.attall(:,1:end)-SINS{1,3}.atttrue(:,1:2:end),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯统计,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
errlen=1;
Rerr=my_getSErr(SINSR{1,3}.Rall(1:end-1,:)'-SINS{1,3}.R(:,1:2:end-2),errlen,2)*1000;        %位置误差单位mm
Atterr=my_getSErr(SINSR{1,3}.attall(:,1:end-1)-SINS{1,3}.atttrue(:,1:2:end-2),errlen,2)/arcdeg*60; %姿态误差单位分
fprintf('纯惯终值,位置误差%f %f %f（mm），姿态误差%f %f %f（分）\n',...
    Rerr(1),Rerr(2),Rerr(3),Atterr(1),Atterr(2),Atterr(3));
beep