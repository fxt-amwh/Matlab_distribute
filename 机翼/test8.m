...
Sinf.Rlist=[2*ones(1,simListLen);
            0*ones(1,simListLen);
            0*ones(1,simListLen)];
Sinf.ulist=1*ones(1,simListLen)*arcdeg;%子惯处挠曲角
Sinf.flist=1*ones(1,simListLen)*0.01;%挠曲频率
Sinf.aerrlist=0.1*randn(3,simListLen)*arcdeg;%子惯安装误差角
Sinf.wim0=MINS.wim0;
SinfCell{1,1}=Sinf;
...
for m=1:M
    atterrList(:,:,m)=zeros(3,simListLen)*arcdeg;%子惯安装误差角
end
%% 子惯反解算数据
