...
nameList=["SINS1","SINS2","SINS3","SINS4","SINS5"];
% 子惯运动数据设计
fprintf('子惯仿真...\n'); 
Sinf.ts=MINS.ts;
len=size(MINS.wim,2);
...
Sinf.aerrlist=[ 0.1  -0.4  0.2  0.4 -0.1;
                0.5   0.2  0.3 -0.2  0.3;
               -0.2   0.7 -0.1  0.3  0.1]*d2g;%子惯安装误差角
...
for m=1:M
    atterrList(:,:,m)=[ 0.13  -0.43  0.23  0.43 -0.13;
                        0.53   0.23  0.33 -0.23  0.33;
                       -0.23   0.73 -0.13  0.33  0.13]*d2g;%子惯安装误差角
end
%% 子惯反解算数据
...
for m=1:M
    for n=1:N
        ...
        beg=ceil(200/nts);
        End=ceil(500/nts);
        inx=abs(SINSFR{m,n}.Rall(1:end,:)'-SINS{m,n}.R(:,1:2:end));
        Rerr=my_getSErr(inx(:,beg:End),size(inx(:,beg:End),2),2)*1000;        %位置误差单位mm
        ina=abs(SINSFR{m,n}.attall(:,1:end)-SINS{m,n}.atttrue(:,1:2:end));
        Atterr=my_getSErr(ina(:,beg:End),size(ina(:,beg:End),2),2)/arcdeg*60; %姿态误差单位分
