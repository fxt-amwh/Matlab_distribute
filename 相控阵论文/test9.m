%% 相控阵位置误差加入后峰值衰减
% 版本时间：2021/01/21 
% 设计初衷：机翼上弯曲变形对蒙皮天线的影响 封装 0.5m处中心安装蒙皮 精细地解算位移 14所建议后
clc
clear
close all
...
flag=2;%控制仿真模式 1：x方向的弯曲变形 2：弯曲变形+随机二维位姿误差 
SigmaMax=0.1*lambda;%控制仿真位置误差最大值
SitaMax=1;%控制仿真俯仰误差最大值
PhiMax=1;%控制仿真方向误差最大值
simLen=100;%蒙特卡洛仿真次数
Zmax=1*(0:0.025:0.5)*lambda;%控制变形指数
umax=Zmax/max(Zmax)*10*pi/180;%控制变形指数
% umax=Zmax/max(Zmax)*0.01;%控制变形指数
% umax=ones(size(umax))*5*pi/180;%控制变形指数
    ...
    for simi=1:simLen
        switch flag
            case 1
                ...
                [d_x,dz]=my_fixdefload(xdir,2,umax(loopi),1);%机翼载荷挠曲模型标准量 精准模型1
%                 umax=-1*pi/180;
%                 [d_x,dz]=my_fixdefload(xdir+ddx,1,umax,1);%机翼载荷挠曲模型标准量 精准模型1
%                 [dx_measure,dz_measure]=my_fixdefload(xdir,5,umax(loopi),1);%机翼载荷挠曲模型补偿量
...
%                 f_5=my_getDirPt(sita,phi,d_x,ddy,dz+ddz);%5°挠曲指数方向图函数
            ...
            case 2
                for loopj=1:len
                    ...
                    xdir=0.5 + (-floor(M/2):1:floor(M/2))*dx;%蒙皮中心安装在 2m 处
                    [d_x,dz]=my_fixdefload(xdir,2,umax(loopj),1);%机翼载荷挠曲模型标准量 精准模型1
                    dz_measure=my_fixdefload(xdir,2,umax(loopj)-0.1*pi/180);%机翼载荷挠曲模型补偿量
                    RO.base.ddx=d_x+ddx;%x方向位置误差矩阵
                    RO.base.ddy=ddy;%y方向位置误差矩阵
                    RO.base.ddz=dz+ddz;%z方向位置误差矩阵
