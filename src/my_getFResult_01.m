function [INSSFR,Filter]=my_getFResult_01(MINS,SINS,KFinit,atterr0,looplen,varargin)
% 函数名称：my_SINSgetFResult_01 
% 函数功能：由子惯输出进行相对导航滤波 新增旋转初始姿态矩阵
% 输入：U      :当前导航系下相对速度
%       varargin:1个输入表示SERR
...
INSSFR.attinitall=zeros(3,looplen);
...
%     qint = qdelphi(m2qua(a2mat(attint)),-kf.Xk(1:3));%尝试旋转初始姿态
    qint = qdelphi(m2qua(a2mat(attint)),[0;0;0]);%尝试旋转初始姿态
    attint=q2att(qint);
    ...
    [rz,rx,ry]=dcm2angle(q2mat(qint),'zxy');%转换到传统的坐标
    INSSFR.attinitall(:,i)=[rx,ry,-rz];
